	...
	var add_edit_events = {
		...
		afterSubmit: function(response, postdata, frmoper){
			...
			if(grid.data('settings').single_record.active && grid.data('settings').single_record.displayAlert)
				if(grid.data('settings').form_sent_button && grid.data('valid'))
					alert($.jset.messages.recordSent);
				else
...
/*		get_form_field: function(formid, name){
		...
		get_form_field: function(formid, name, cache_it){
			...
			if(!value.length)
				console.log('Warning - field: ' + name + ' was not found in form:', formid);
		...
		get_filterToolbar: function(grid){
			var exclude = $("div.ui-jqgrid[id^='gbox_'] tr.ui-search-toolbar", $.jset.fn.get_grid_container(grid));
			return $('tr.ui-search-toolbar', $.jset.fn.get_grid_container(grid)).not(exclude);
		},
			...
			if (grid.data('settings').filterToolbar.navButtonAdd){
				var options = {
					...
					onClickButton:function(){
						...
						grid.jqGrid('setGridHeight', $.jset.fn.get_filterToolbar(grid).is(':visible') ? height - 23 : height + 23);
					}
				};
...
//									obj.value = col.values ? $.extend({}, value, col.values) : value;
		...
		edit_row: function(grid_id, id){
			var grid = $(grid_id);
			var options = grid.data('settings').navigation.edit;
			grid.data('settings').navigation.options.editfunc.call(grid, id, options);
		},
		delete_row: function(grid_id, id){
			var grid = $(grid_id);
